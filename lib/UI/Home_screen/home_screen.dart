...
class FoodOrderingScreenViewState extends State<FoodOrderingScreenView> {
  ...
  void initState() {
    super.initState();
    if (kIsWeb) {
      printerService = MockPrinterService(); // Web-safe mock
    } else {
      // Use try-catch to avoid crashing when dart:io isn't available
      try {
        if (Platform.isAndroid) {
          printerService = RealPrinterService();
        } else {
          printerService = MockPrinterService(); // fallback
        }
      } catch (e) {
        printerService = MockPrinterService(); // fallback in case of error
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    Widget mainContainer() {
      ...
      void updateBillingList(String productId) {
        ...
        final index =
            billingItems.indexWhere((item) => item['_id'] == productId);
                                                                ...
                                                                onPressed: () {
                                                                  setState(() {
                                                                    if (p.counter >
                                                                      ...
                                                                      p.counter--;
                                                                      debugPrint(
                                                                          "counterDecre: ${p.counter}");

                                                                      final existingIndex = billingItems.indexWhere((item) =>
                                                                          item[
                                                                              '_id'] ==
                                                                          p.id);
                                                                      if (existingIndex !=
